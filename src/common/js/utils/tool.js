...
// copy
const copy = (source) => {
    if (toString.call(source) !== coreType['core_obj'] && toString.call(source) !== coreType['core_array']) {
		throw new Error('type is error');
	}
	let tempObj = toString.call(source) === coreType['core_array']
		? []
		: {};
	for (let i in source) {
		if (hasOwn.call(source, i)) {
			if (toString.call(source[i]) === coreType['core_array']) {
				tempObj[i] = copy(source[i]);
			} else {
				tempObj[i] = source[i];
			}
		}
